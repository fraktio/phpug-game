    ...
    '/map': ->
        ...
        $.ajax({
            ...
            success: (data)->
                ...
                w = window.innerWidth
                h = window.innerHeight
                document.body.querySelector('#canvas-container').innerHTML = '<canvas width="' + w + '" height="' + h + '" style="width: ' + w + 'px; height: ' + h + 'px;"></canvas>'
                c = document.getElementsByTagName('canvas')[0]
                ctx = c.getContext '2d'
                players = {}
                images = {}
                ...
                for id, playerImage in assets.players
                    image = new Image()
                    image.src = assetsUrl + '/' +playerImage
                    images[id] = image


                for obstacleImage in assets.obstacles
                    image = new Image()
                    image.src = assetsUrl + '/' +obstacleImage
